...
def extraction_routine(args, tract, patch, filename, savedir):
    forced_photometry_tables = celib.read_forced_photometry(
        tract = tract,
        patch = patch,
        SNR_minimum=args.snr_min,
...
def main():
    ...
    for tract, patch in available_tracts_patches:
        ...
        toc = time.perf_counter()
        print(f"Finished tract = {tract} patch = {patch} in {(toc-tic)/3600} hours")
